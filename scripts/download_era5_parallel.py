...
用法:
    python download_era5_parallel.py --start_year 2000 --end_year 2000 --workers 8
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('era5_download_parallel.log'),
...
PROGRESS_FILE = BASE_DIR / "download_progress_parallel.json"
...
class ERA5ParallelDownloader:
    def __init__(self, start_year, end_year, workers=8):
    ...
    def download_year(self, year):
        """并行下载一年的数据"""
        logger.info(f"开始并行下载 {year} 年数据 (workers={self.workers})")
    ...
    def run(self):
        """主运行函数"""
        logger.info(f"开始并行下载 ERA5 数据")
        logger.info(f"年份范围: {self.start_year} - {self.end_year}")
        logger.info(f"并行数: {self.workers}")
...
def main():
    parser = argparse.ArgumentParser(description='ERA5并行下载脚本')
    ...
    parser.add_argument('--workers', type=int, default=8, help='并行下载数 (默认: 8)')

    args = parser.parse_args()

    downloader = ERA5ParallelDownloader(args.start_year, args.end_year, args.workers)
    downloader.run()
