...
### 存储位置
- **所有数据必须存储在F盘**：`F:\` 目录
- **其他盘没有空间**，禁止在C盘、D盘、E盘存储数据

### 数据目录结构
```
F:\
├── ERA5再分析数据下载\    # ERA5原始数据
│   ├── pressure_level\   # 压力层数据
│   └── single_level\     # 单层数据
├── GMCP_Precipitation\   # GMCP降水数据（已有）
└── weather_data\         # 预处理后的数据
    ├── train\            # 训练集
    ├── val\              # 验证集
    └── test\             # 测试集
```
...
### 按需下载策略
1. **分阶段下载**：不一次性下载全部，按训练需求分批下载
2. **边下边训**：下载一部分，训练一部分，提高效率
3. **优先级排序**：先下载2000-2010年数据用于初始训练

### 下载优先级
```
阶段1（立即下载）: 2000-2005年数据
阶段2（训练中下载）: 2006-2015年数据
阶段3（后期下载）: 2016-2024年数据
```
